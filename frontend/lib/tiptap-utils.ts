...
export const handleImageUpload = async (
  ...
  const resp = await fetch(`${process.env.NEXT_PUBLIC_API_URL!}/post/${post.id}/upload_file`, {
    signal: abortSignal,
    method: "POST",
  ...
  const respJson: FileUploadResponse = await resp.json();
  if (!respJson.success) {
    throw new Error("Could not upload file");
  }

  return respJson.file.url;
};
